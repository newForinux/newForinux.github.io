...
Material-UI의 컴포넌트를 적극 사용했다. ~~(때문에 CSS는 거의 만지지 않게 되었다는 건 슬픈 일이야...)~~
...
<br>
처음에 저 `...`을 보고 굉장히 당황했는데,

`spread operator`, 즉 전개 구문이다. (JS es6에서 추가된 문법이라고 한다)

배열의 값을 받아와 뿌려놓거나, 저 위처럼 배열을 확장해 여러 일을 할 수도 있다고.

궁금해서 `...rawTheme.typography`에서 `...`을 지우면 오류가 난다.
...
눈물을 머금고 JS로 돌린 첫 컴포넌트가 되겠다. (첫 단계부터ㅠ)
...
![Main Layout](/assets/img/blog/layouts.png){:.border.lead width="676" height="530" loading="lazy"}
...
![Sincerity](/assets/img/blog/always.gif){:.border.lead width="460" height="259" loading="lazy"}
...
함수형 컴포넌트에 `props`로 넘겨주는 방법이 있었는데, 그 타입을 알 리가 있나...
...
이 인터페이스를 props의 타입으로 선언해주면 말끔히 돌아간다.
